(function(root, factory) {
  ...
  function normalizePrintStyle(value) {
    ...
    return {
      page_margin_mm: numberInRange(value.page_margin_mm, 8, 3, 20),
      logo_width_mm: numberInRange(value.logo_width_mm, 30, 16, 60),
      logo_height_mm: numberInRange(value.logo_height_mm, 11, 6, 28),
      body_font_px: numberInRange(value.body_font_px, 6.6, 4.8, 11),
      table_font_px: numberInRange(value.table_font_px, 6.4, 4.8, 10),
      header_gap_mm: numberInRange(value.header_gap_mm, 12, 4, 28),
      document_width_mm: numberInRange(value.document_width_mm, 190, 90, 210),
      header_meta_width_mm: numberInRange(value.header_meta_width_mm, 22, 14, 36),
      header_gap_column_mm: numberInRange(value.header_gap_column_mm, 7, 2, 24),
      table_cell_padding_y_mm: numberInRange(value.table_cell_padding_y_mm, 1.4, 0.6, 4),
      table_cell_padding_x_mm: numberInRange(value.table_cell_padding_x_mm, 1, 0.5, 3)
    };
  }
  ...
  function pageRule(style) {
    return "@page{margin:" + style.page_margin_mm + "mm;}";
  }
  ...
  function baseStyles(style) {
    return (
      ...
      ".hpe-col-code{width:16%;}" +
      ".hpe-col-name{width:34%;}" +
      ".hpe-col-qty{width:9%;text-align:center;}" +
      ".hpe-col-unit{width:10%;text-align:center;}" +
      ".hpe-col-vat{width:8%;text-align:center;}" +
      ".hpe-col-explanation{width:23%;}" +
      ...
      ".hpe-page{padding:0;}" +
      ".hpe-slip-header{grid-template-columns:minmax(0,1fr) minmax(0,1fr) var(--hpe-header-meta-width);gap:var(--hpe-header-column-gap);}" +
      ".hpe-items{width:100%;}" +
      "}" +
